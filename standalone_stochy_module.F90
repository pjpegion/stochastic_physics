...
implicit none
private
public GFS_diag_type,GFS_control_type,GFS_statein_type,GFS_init_type,GFS_grid_type,GFS_coupling_type
integer,public :: isc,jsc,iec,jec,isd,ied,jsd,jed

type GFS_diag_type
    real (kind=kind_phys),public, allocatable :: ca_out  (:)      !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_deep  (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_turb  (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_shal  (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_rad   (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_micro (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca1   (:)  
    real (kind=kind_phys),public, allocatable :: ca2   (:)  
    real (kind=kind_phys),public, allocatable :: ca3   (:)  
end type GFS_diag_type
type GFS_control_type
   integer,public  :: levs,me,nx,ny
   integer,public,allocatable  :: blksz(:)        !< for explicit data blocking: block sizes of all blocks
   real(kind=kind_phys),public :: dtp             !< physics timestep in seconds
   real(kind=kind_phys),public :: phour           !< previous forecast hour
   real(kind=kind_phys),public :: sppt_amp        !< amplitude of sppt (to go to cld scheme)
   integer,public              :: kdt             !< current forecast iteration
   logical,public  :: do_sppt,do_shum,do_skeb,do_sfcperts,use_zmtnblck,do_rnda
   integer,public  ::  skeb_npass,nsfcpert
   character(len=65),public :: fn_nml                   !< namelist filename
   character(len=256),public,allocatable :: input_nml_file(:) !< character string containing full namelist
   real(kind=kind_phys),public ::  pertz0(5),pertzt(5),pertshc(5),pertlai(5),pertvegf(5),pertalb(5)
 !---cellular automata control parameters
    integer,public              :: nca             !< number of independent cellular automata
    integer,public              :: nlives          !< cellular automata lifetime
    integer,public              :: ncells          !< cellular automata finer grid
    real(kind=kind_phys),public :: nfracseed       !< cellular automata seed probability
    integer,public              :: nseed           !< cellular automata seed frequency
    logical,public              :: do_ca           !< cellular automata main switch
    logical,public              :: ca_sgs          !< switch for sgs ca
    logical,public              :: ca_global       !< switch for global ca
    logical,public              :: ca_smooth       !< switch for gaussian spatial filter
    logical,public              :: isppt_deep      !< switch for combination with isppt_deep. OBS! Switches off SPPT on other tendencies!
    logical,public              :: isppt_pbl
    logical,public              :: isppt_shal      !
    logical,public              :: pert_flux       !
    logical,public              :: pert_trigger    !
    integer,public              :: iseed_ca        !< seed for random number generation in ca scheme
    integer,public              :: ca_amplitude    !< seed for random number generation in ca scheme
    integer,public              :: nspinup         !< number of iterations to spin up the ca
    real(kind=kind_phys),public :: nthresh         !< threshold used for perturbed vertical velocity
end type GFS_control_type

type GFS_statein_type
   real (kind=kind_phys),public, allocatable :: pgr  (:)      !< surface pressure (Pa) real
   real (kind=kind_phys),public, allocatable :: qgrs (:,:,:)  !< layer mean tracer concentration
   real (kind=kind_phys),public, allocatable :: vvl  (:,:)    !< layer mean vertical velocity in pa/sec
   real (kind=kind_phys),public, allocatable :: prsl  (:,:)   !< model layer mean pressure Pa
end type GFS_statein_type


type GFS_init_type
   integer :: nlunit
   real(kind=kind_phys),public,allocatable :: ak(:),bk(:),xlon(:,:),xlat(:,:)
   integer,public,allocatable  :: blksz(:)        !< for explicit data blocking: block sizes of all blocks
end type GFS_init_type

type GFS_grid_type
    real (kind=kind_phys),public,allocatable :: xlat   (:)    !< grid latitude in radians, default to pi/2 ->
    real (kind=kind_phys),public,allocatable :: xlon   (:)    !< grid longitude in radians, default to pi/2 ->
end type GFS_grid_type

type GFS_coupling_type
    real (kind=kind_phys),public,allocatable :: shum_wts  (:,:)
    real (kind=kind_phys),public,allocatable :: sppt_wts  (:,:)
    real (kind=kind_phys),public,allocatable :: sppt_pattern(:)
    real (kind=kind_phys),public,allocatable :: skebu_wts (:,:)
    real (kind=kind_phys),public,allocatable :: skebv_wts (:,:)
    real (kind=kind_phys),public,allocatable :: sfc_wts   (:,:)
    real (kind=kind_phys),public, allocatable :: cape    (:)      !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_out  (:)      !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_deep  (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_turb  (:)     !< cellular automata fraction
    real (kind=kind_phys),public, allocatable :: ca_shal  (:)     !< cellular automata fraction
    ...
    real (kind=kind_phys),public, allocatable :: ca2   (:)  
    real (kind=kind_phys),public, allocatable :: ca3   (:)  
    integer,public              :: nsfcpert=6                             !< number of sfc perturbations
